...
class MySqlNpcRepository extends INpcRepository {
    ...
    async loadAll(connection = null) {
        const query = 'SELECT npc_id as id, name, trust, fear, memory_health, mood, activity FROM npc_profiles';
        try {
    ...
    async saveState(npc, connection = null) {
        const query = `
            INSERT INTO npc_profiles (npc_id, name, trust, fear, memory_health, mood, activity)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        `;
        const params = [npc.id, npc.name, npc.trust, npc.fear, npc.memory_health, npc.mood, npc.activity];
